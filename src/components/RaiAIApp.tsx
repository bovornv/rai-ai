...
import { Home, Scan, MapPin, Menu, Plus, Store, DollarSign } from "lucide-react";
...
type MainView = 'today' | 'scan' | 'fields' | 'sell';
type DrawerView = 'help' | 'settings' | 'counter';
...
const RaiAIApp = () => {
  ...
  const renderMainContent = () => {
    ...
    switch (currentView) {
      ...
      case 'sell':
        return <SellPricesPage onBack={() => setCurrentView('today')} selectedCrop={appState.selectedCrop} />;
      default:
        return null;
    }
  };
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="flex items-center justify-between p-4 border-b bg-card">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-primary">{t('raiAI')}</h1>
        </div>
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button 
              variant="ghost" 
              size="icon" 
              className="hover:bg-accent border border-gray-300"
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-card border-t">
          <div className="flex">
            ...
            >
              ...
              <span className="text-xs">{t('sell')}</span>
            </Button>
          </div>
        </nav>
      )}
